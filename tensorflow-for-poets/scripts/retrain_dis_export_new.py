...
def main(_):
  ...
  with tf.device(device_func):
    ...
    if is_chief:
      ...
      tf.logging.info(FLAGS.output_graph)
    ...
    if is_chief:
      graph = load_graph(FLAGS.output_graph)
      ...
      label_file = FLAGS.output_labels
      ...
      with tf.Session(graph=graph,config=tf.ConfigProto(
        ...
        export_path = FLAGS.serve_model_path
        ...
        tensor_info_z = tf.saved_model.utils.build_tensor_info(tf.convert_to_tensor(labels))
