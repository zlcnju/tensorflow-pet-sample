...
def main(_):
  ...
  with tf.device(device_func):
    ...
    with tf.Session(
            target, graph=graph, config=tf.ConfigProto(log_device_placement=True)) as sess:
      ...
      #tensor_info_x = tf.saved_model.utils.build_tensor_info(bottleneck_input)
      #tensor_info_y = tf.saved_model.utils.build_tensor_info(ground_truth_input)
