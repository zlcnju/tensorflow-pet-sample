...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
                           bottleneck_tensor_size, device_func, global_step):
  ...
  with tf.device(device_func):
    ...
    with tf.name_scope('train'):
      ...
      train_step = optimizer.minimize(cross_entropy_mean, global_step)
...
def prepare_file_system():
  ...
  if tf.gfile.Exists(FLAGS.tensorboard):
    tf.gfile.DeleteRecursively(FLAGS.tensorboard)
  tf.gfile.MakeDirs(FLAGS.tensorboard)
...
def main(_):
  ...
  tf_config_json = os.environ.get(FLAGS.env_type, "{}")
  ...
  logging.info("-----device_func : %s", device_func)
  ...
  with tf.device(device_func):
    ...
    is_chief = (job_type in ["master", "chief"])

    if is_chief:
        FLAGS.bottleneck_dir = 'tf_files/bottlenecks-chief'
    with graph.as_default():
        ...
        final_tensor) = add_final_training_ops(
            ...
            model_info['bottleneck_tensor_size'], device_func, global_step)

        # Create the operations we need to evaluate the accuracy of our new layer.
        evaluation_step, prediction = add_evaluation_step(
            final_tensor, ground_truth_input)

        # Merge all the summaries and write them out to the tensorboard
    ...
    sv = tf.train.Supervisor(is_chief=is_chief, graph=graph, recovery_wait_secs=1, global_step=global_step)

    if is_chief:
        print('Worker %d: Initailizing session...')
    else:
        print('Worker %d: Waiting for session to be initaialized...')
    sess = sv.prepare_or_wait_for_session(server.target, config=tf.ConfigProto(log_device_placement=True))
    ...
    train_writer = tf.summary.FileWriter(FLAGS.tensorboard + '/train',
                                    sess.graph)

    validation_writer = tf.summary.FileWriter(FLAGS.tensorboard + '/validation')

    
    g_step = 0
    # Run the training for as many cycles as requested on the command line.
    for step in range(FLAGS.how_many_training_steps):
        if g_step + 1 > FLAGS.how_many_training_steps:
          break
        ...
        train_summary, _, g_step = sess.run(
        ...
        print('----g_step: %d', g_step)
        print('----step: %d', step)
        # Every so often, print out how well the graph is training.
        is_last_step = (g_step + 1 == FLAGS.how_many_training_steps)
        if (step % FLAGS.eval_step_interval) == 0 or is_last_step:
            ...
            tf.logging.info('%s: Step %d: Train accuracy = %.1f%%' %
                            (datetime.now(), step, train_accuracy * 100))
            tf.logging.info('%s: Step %d: Cross entropy = %f' %
                            (datetime.now(), step, cross_entropy_value))
        ...
        if is_chief:
          ...
          if (intermediate_frequency > 0 and (step % intermediate_frequency == 0)
              and step > 0):
              intermediate_file_name = (FLAGS.intermediate_output_graphs_dir +
                                      'intermediate_' + str(step) + '.pb')
    ...
    if is_chief:
      with graph.as_default():
        ...
        export_path = FLAGS.model
        print('Exporting trained model to', export_path)
        if gfile.Exists(export_path):
            gfile.DeleteRecursively(export_path)
        ...
        tensor_info_z = tf.saved_model.utils.build_tensor_info(tf.convert_to_tensor(image_lists.keys()))

        prediction_signature = (
        tf.saved_model.signature_def_utils.build_signature_def(
            inputs={"images": tensor_info_x},
            outputs={"scores": tensor_info_y,
                    "labels": tensor_info_z
            },
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--model',
      type=str,
      default='/hostmount/catmodel/1',
      help="""\
      A model path to export model for serving.\
      """
  )
  parser.add_argument(
      '--bottleneck_dir',
